...
export default function Saque() {
  ...
  const { refreshSaldo, refreshUsuario } = useContext(AuthContext);
  ...
  const syncUsuarioLocal = async (fallbackUsuario = null) => {
    const atualizado = (await refreshUsuario()) || fallbackUsuario;
    if (atualizado && typeof atualizado.saldo === 'number') {
      await refreshSaldo();
    }
  };
  ...
  const handleSaque = async () => {
    ...
    if (!Number.isFinite(valorNumerico) || valorNumerico < MINIMO) {
      adicionarToast(`O valor mínimo para saque é de R$ ${MINIMO.toFixed(2)}.`, 'erro');
      return;
    }

    const saldoAtual = obterSaldoLocal();
    if (valorNumerico > saldoAtual) {
      adicionarToast(`Saldo insuficiente. Saldo disponível: R$ ${saldoAtual.toFixed(2)}.`, 'erro');
      return;
    }
    ...
    try {
      ...
      await syncUsuarioLocal(usuarioAtualizado);

      adicionarToast('Pedido de saque registrado com sucesso! O processamento será feito pela nossa equipe/gateway.', 'sucesso');
    ...
    <Container>
      <Card>
        ...
        <Campo>
          ...
          <Select value={metodo} onChange={(e) => setMetodo(e.target.value)} disabled>
