...
export default function Deposito() {
  ...
  const handleDeposito = async () => {
    ...
    if (!Number.isFinite(valorNumerico) || valorNumerico < MINIMO) {
      adicionarToast(`O valor mínimo para depósito é de R$ ${MINIMO.toFixed(2)}.`, 'erro');
      return;
    }
    ...
    try {
      ...
      const img = res.data?.qrCodeBase64 || res.data?.qrCode || res.data?.qrCodeUrl || null;
      const copia = res.data?.pixCopiaCola || res.data?.copiaECola || res.data?.payload || '';
